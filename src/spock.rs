...
impl SpockInstance for Instance {
    ...
    fn enumerate_all_physical_devices(self) -> Result<Vec<PhysicalDevice>, Error> {
        unsafe { self.enumerate_physical_devices(try!(self.count_physical_devices())) }
    }
}
...
impl SpockPhysicalDevice for PhysicalDevice {
    ...
    fn get_format_properties(self, format: Format) -> FormatProperties {
        unsafe {
            let mut properties: FormatProperties;
    ...
    fn get_image_format_properties(self, format: Format, image_type: ImageType, tiling: ImageTiling, usage_flags: ImageUsageFlags, create_flags: ImageCreateFlags) -> Result<ImageFormatProperties, Error> {
        unsafe {
            let mut properties: ImageFormatProperties;
    ...
    fn get_properties(self) -> PhysicalDeviceProperties {
        unsafe {
            let mut properties: PhysicalDeviceProperties;
    ...
    fn get_sparse_image_format_properties(self, format: Format, image_type: ImageType, sample_count: SampleCountFlags, usage: ImageUsageFlags, tiling: ImageTiling, count: u32) -> Vec<SparseImageFormatProperties> {
        unsafe {
            ...
            vkGetPhysicalDeviceSparseImageFormatProperties(self, format, image_type, sample_count, usage, tiling, &mut count, &mut properties);
            properties
        }
    }
...
pub trait SpockDevice {
    ...
    fn get_memory_commitment(self, DeviceMemory) -> Result<DeviceSize, Error>;
    ...
    fn get_buffer_memory_requirements(self, Buffer) -> Result<MemoryRequirements, Error>;
    fn get_image_memory_requirements(self, Image) -> Result<MemoryRequirements, Error>;
    fn get_image_sparse_memory_requirements_count(self, Image) -> u32;
    fn get_image_sparse_memory_requirements(self, Image, count: u32) -> Result<MemoryRequirements, Error>;
    fn get_all_image_sparse_memory_requirements(self, Image) -> Result<MemoryRequirements, Error>;
    ...
    fn merge_pipeline_caches(self, Vec<PipelineCache>) -> Result<PipelineCache, Error>;
    ...
    fn update_descriptor_sets(self, Vec<WriteDescriptorSet>, Vec<CopyDescriptorSet>) -> Error;
...
impl SpockDevice for Device {
    ...
    fn get_queue(self, queue_family_index: u32, queue_index: u32) -> Result<Queue, Error> {
        unsafe {
            ...
            let result = vkGetDeviceQueue(self, queue_family_index, queue_index);
    ...
    fn flush_mapped_memory_ranges(self, ranges: Vec<MappedMemoryRange>) -> Error {
        unsafe { vkFlushMappedMemoryRanges(self, ranges.len() as u32, &ranges) }
    }

    fn invalidate_mapped_memory_ranges(self, ranges: Vec<MappedMemoryRange>) -> Error {
        unsafe { vkInvalidateMappedMemoryRanges(self, ranges.len() as u32, &ranges); }
    }

    fn get_memory_commitment(self, memory: DeviceMemory) -> Result<DeviceSize, Error> {
        unsafe {
            let mut commitment: DeviceSize;
            let result = vkGetDeviceMemoryCommitment(self, memory, &mut commitment);
            vulkan_result!(result, commitment)
        }
    }
    ...
    fn get_buffer_memory_requirements(self, buffer: Buffer) -> Result<MemoryRequirements, Error> {
        unsafe {
            let mut requirements: MemoryRequirements;
            let result = vkGetBufferMemoryRequirements(self, buffer, &mut requirements);
            vulkan_result!(result, requirements)
        }
    }

    fn get_image_memory_requirements(self, image: Image) -> Result<MemoryRequirements, Error> {
        unsafe {
            let mut requirements: MemoryRequirements;
            let result = vkGetImageMemoryRequirements(self, image, &mut requirements);
            vulkan_result!(result, requirements)
        }
    }
    ...
    fn get_image_sparse_memory_requirements(self, image: Image, count: u32) -> Result<MemoryRequirements, Error> {
        unsafe {
            let mut mut_count = count;
            let mut requirements: MemoryRequirements;
            let result = vkGetImageSparseMemoryRequirements(self, image, &mut mut_count, &mut requirements);
            vulkan_result!(result, requirements)
        }
    }

    fn get_all_image_sparse_memory_requirements(self, image: Image) -> Result<MemoryRequirements, Error> {
        unsafe { self.get_image_sparse_memory_requirements(image, self.get_image_sparse_memory_requirements_count()); }
    }
    ...
    fn wait_for_fences(self, fences: Vec<Fence>, wait_for_all: bool, timeout: u64) -> Error {
        unsafe { vkWaitForFences(self, fences.len() as u32, fences.as_ptr(), wait_for_all as u64, timeout) }
    }
    ...
    fn get_image_subresource_layout(self, image: Image, subresource: ImageSubresource) -> SubresourceLayout {
        unsafe {
            let mut layout: SubresourceLayout = ptr::null_mut();
    ...
    fn merge_pipeline_caches(self, pipeline_caches: Vec<PipelineCache>) -> Result<PipelineCache, Error> {
        unsafe {
            let mut new_pipeline_cache: PipelineCache = ptr::null_mut();
            let result = vkMergePipelineCaches(self, &mut new_pipeline_cache, pipeline_caches.len() as u32, pipeline_caches.as_mut_ptr());
            vulkan_result!(result, new_pipeline_cache)
        }
    }
    ...
    fn create_graphics_pipelines(self, pipeline_cache: PipelineCache, create_infos: Vec<GraphicsPipelineCreateInfo>, allocator_opt: Option<AllocationCallbacks>) -> Result<Vec<Pipeline>, Error> {
        unsafe {
            ...
            pipelines.resize(create_infos.len(), Pipeline{..Default::default()});
    ...
    fn allocate_descriptor_set(self, allocate_info: DescriptorSetAllocateInfo) -> Result<Vec<DescriptorSet>, Error> {
        unsafe {
            ...
            descriptor_sets.resize(allocate_info.descriptorSetCount as usize, ptr::null());

            let result = vkAllocateDescriptorSets(self, &allocate_info, &mut descriptor_sets);
    ...
    fn update_descriptor_sets(self, writes: Vec<WriteDescriptorSet>, copies: Vec<CopyDescriptorSet>) -> Error {
    ...
    fn get_render_area_granularity(self, render_pass: RenderPass) -> Extent2D {
        unsafe {
            ...
            vkGetRenderAreaGranularity(self, render_pass, &extent);
            extent
        }
    }
    ...
    fn allocate_command_buffers(self, allocate_info: CommandBufferAllocateInfo) -> Result<Vec<CommandBuffer>, Error> {
        unsafe {
            ...
            buffers.resize(allocate_info.commandBufferCount as usize, ptr::null());

            let result = vkAllocateCommandBuffers(self, allocate_info, &mut buffers);
...
pub trait SpockQueue {
    ...
    fn wait_idle(self) -> Error;
    fn bind_sparse(self, Queue, Vec<BindSparseInfo>, Fence) -> Error;
}
...
impl SpockQueue for Queue {
    ...
    fn bind_sparse(self, queue: Queue, bind_info: Vec<BindSparseInfo>, fence: Fence) -> Error {
        unsafe { vkQueueBindSparse(self, queue, bind_info.len() as u32, bind_info.as_ptr(), fence) }
    }
}
...
pub trait SpockCommandBuffer {
    ...
    fn cmd_wait_events(self, Vec<Event>, PipelineStageFlags, PipelineStageFlags, Vec<MemoryBarrier>, Vec<MemoryBarrier>, Vec<ImageMemoryBarrier>);
    fn cmd_pipeline_barrier(self, PipelineStageFlags, PipelineStageFlags, DependencyFlags, Vec<MemoryBarrier>, Vec<MemoryBarrier>, Vec<ImageMemoryBarrier>);
...
impl SpockCommandBuffer for CommandBuffer {
    ...
    fn cmd_clear_attachments(self, attachments: Vec<ClearAttachment>, rects: Vec<ClearRect>) {
        unsafe { vkCmdClearAttachments(self, attachments.len() as u32, attachments.as_ptr(), rects.len() as u32, rects.as_prt()); }
    }
    ...
    fn cmd_wait_events(self, events: Vec<Event>, source_mask: PipelineStageFlags, destination_mask: PipelineStageFlags, memory_barriers: Vec<MemoryBarrier>, buffer_memory_barriers: Vec<MemoryBarrier>, image_memory_barriers: Vec<ImageMemoryBarrier>) {
        unsafe { vkCmdWaitEvents(self, events.len() as u32, events.as_prt(), source_mask, destination_mask, memory_barriers.len() as u32, memory_barriers.as_ptr(), buffer_memory_barriers.len() as u32, buffer_memory_barriers.as_ptr(), image_memory_barriers.len() as u32, image_memory_barriers.as_ptr()); }
    }

    fn cmd_pipeline_barrier(self, source_mask: PipelineStageFlags, destination_mask: PipelineStageFlags, dependency_flags: DependencyFlags, memory_barrier: Vec<MemoryBarrier>, buffer_memory_barrier: Vec<MemoryBarrier>, image_memory_barrier: Vec<ImageMemoryBarrier>) {
